...
- Tiers: the free app is just "Offhand", the paid tier "Offhand Pro" — yearly with a 14-day free trial and a one-time lifetime purchase, no monthly. US is the base storefront with manual per-country prices in the consoles. Pro features: custom note styles, PDF/DOCX export, smart calendar suggestions, audio import. The rule for every one of them: the feature stays visible and tappable, a gold crown (`ProCrown` composable over `ic_pro_crown.xml`, SwiftUI `ProCrown` over `ProCrown.imageset`, same SVG, gold is never theme-tinted) marks it while free, and the paywall opens on the committing action, never unprompted. Once Pro, every crown disappears.
...
- Debug override: the platform store is bound under the `PLATFORM_PRO_STORE` qualifier and wrapped by `DebugOverrideProStore`, driven by the debug-only `proOverride` preference (STORE / FREE / PRO, the "Pro status override" segmented control at the bottom of Settings in debug builds). FREE hides any purchase, PRO grants lifetime, and a purchase made while overridden flips it to PRO and returns sample offers, so the whole paywall path can be walked on a dev install the real stores do not know. The uitest fake (`FakeProStore`, lifetime) is bound under the same qualifier and `08-paywall.yaml` walks the free path through the override. `Offhand.storekit` is attached to the dev and uitest schemes for the simulator (StoreKit configuration files only apply to runs started from Xcode). `BuildInfo.isDebugBuild` on iOS comes from the Swift `#if DEBUG` through `IosPlatformDeps`, not from `Platform.isDebugBinary`: the shared framework is always the release XCFramework, which had silently hidden every debug-only feature on iOS. A SwiftUI `Toggle` row in a Form only flips on the switch itself, so the smart-suggestions row is a Button around a hit-testing-disabled Toggle (Maestro taps the label); Android's `SwitchRow` is `toggleable` on the whole row for the same reason.
