...
## Audio import
- Entry points on Android: the upload icon in the notes list (SAF `OpenDocument` with `audio/*`) and an `ACTION_SEND` intent filter on `MainActivity`. Both go through `AudioImportIntake`, which copies the content URI into `cacheDir/imports/` first, because the URI grant belongs to the receiving activity and the decoding runs in the foreground service; the decoder deletes the copy when done. `ImportAudioUseCase` checks the `audioImportUnlocked` entitlement, then `RecordingProcessController.importAudio` (service on Android, direct on iOS).
...
- `MediaCodecAudioDecoder` (androidMain) decodes whatever `MediaExtractor`/`MediaCodec` handle, downmixes and resamples per buffer with `PcmResampler` (core:audio, box low-pass + linear interpolation; commonTest covers the tone) to 16 kHz mono PCM16. iOS has `UnavailableAudioDecoder` until the AVFoundation decoder lands; do not expose the import entry point in Swift before that.
- Shell-side `am start -a SEND` with a MediaStore URI cannot grant the app access (the grant must come from an app), so the share path is verified by sharing from the Files app; the Maestro flow uses the picker path and needs `.maestro/assets/import-sample.m4a` in Download (CI pushes it).
