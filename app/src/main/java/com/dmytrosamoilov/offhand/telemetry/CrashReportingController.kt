...
class CrashReportingController(
    ...
    private fun applyConsent(isGranted: Boolean) {
        if (!isGranted) {
            ...
            // to switch off.
            FirebaseReporting.instanceOrNull(context)?.setCrashlyticsCollectionEnabled(false)
            return
        }
        FirebaseReporting.initialize(context)?.setCrashlyticsCollectionEnabled(true)
    }
}
